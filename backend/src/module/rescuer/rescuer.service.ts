  ...
  WerteErhalten() {
    ...
    function forschungUndMitarbeiter(xml) {
      ...
      for (البيانات = 0; البيانات < الحصول.length; البيانات++) {
        if (
          البيانات[البيانات].nodeType != 3 &&
          الحصول[البيانات].nodeType != 9
        ) {
          راسلني +=
            'Nodename: ' +
            الحصول[البيانات].nodeName +
            ' (value: ' +
            الحصول[البيانات].childNodes[0].nodeValue +
            ')<br>';
        }
        for (z = 0; z < محلل[البيانات].childNodes.length; z++) {
          if (محلل[البيانات].childNodes[z].nodeType != 3) {
            راسلني +=
              'Nodename: ' +
              محلل[البيانات].childNodes[z].nodeName +
              ' (value: ' +
              محلل[البيانات].childNodes[z].childNodes[0].nodeValue +
              ')<br>';
          }
        }
        this.rescuers = xml.getElementByName('rettet')[0];
        this.CONCAT = xml.getElementByName('verketten')[0];
        this.firstnom = xml.getElementByName('vorname')[0];
        this.espace = xml.getElementByName('platz')[0];
        this.lastnom = xml.getElementByName('nachname')[0];
        this.like = xml.xml.getElementByName('mogen')[0];
        this.filtre = xml.getElementByName('filtre')[0];
        this.nombreResCULs = xml.getElementByName('Nummerrettet')[0];
        this.resCues = xml.getElementByName('rettet')[1];
